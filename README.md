...
|------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--|
| [01_scRNA_analysis_preprocessing.ipynb](notebooks/01_scRNA_analysis_preprocessing.ipynb)   | End to end workflow, where we understand the cells, run ETL on the data set then visualize and explore the results. <br>This tutorial is good for all users | 24GB /<br>Standard RSC Instance |
| [02_scRNA_analysis_extended.ipynb](notebooks/02_scRNA_analysis_extended.ipynb)   | This notebook continues from the outputs of [01_scRNA_analysis_preprocessing.ipynb](notebooks/01_scRNA_analysis_preprocessing.ipynb) as an overview of methods that can be used to investigate transcriptional regulation | 24GB /<br>Standard RSC Instance |
...
| [04_scRNA_analysis_dask_out_of_core.ipynb](notebooks/04_scRNA_analysis_dask_out_of_core.ipynb) | In this notebook, we show the scalability of the analysis to up to 11M cells easily by using Dask.<br>**Requires a 48GB GPU** | 48GB /<br>Standard RSC Instance |
| [05_spatial_demo.ipynb](notebooks/05_spatial_demo.ipynb) | GPU-accelerated spatial analysis using rapids-singlecell and Squidpy. Covers spatial autocorrelation (Moran's I and Geary's C) and co-occurrence analysis to reveal tissue organization and cell-cell interactions. | 24GB /<br>Standard RSC Instance |
| [06_scRNA_analysis_1.0M_brain_example.ipynb](notebooks/06_scRNA_analysis_1.0M_brain_example.ipynb) | End to end analysis of 1M brain cells from 10X Genomics, following a similar workflow to [01_scRNA_analysis_preprocessing.ipynb](notebooks/01_scRNA_analysis_preprocessing.ipynb).<br>**Requires an 80GB GPU, like an H100** | 80GB /<br>Large RSC Instance |
| [07_perturbation_analysis_brain.ipynb](notebooks/07_perturbation_analysis_brain.ipynb) | GPU-accelerated perturbation analysis on the world's largest in vivo brain CRISPR atlas (7M+ cells, thousands of genetic perturbations) from PerturbAI. Uses Dask/RAPIDS to load and process data at full resolution, builds PCA embeddings, computes E-distance effect sizes across gene and cell type pairs, and visualizes a global perturbation-response map.<br>**Requires 8x H100s or better** | 80GB /<br>Large RSC Instance |
